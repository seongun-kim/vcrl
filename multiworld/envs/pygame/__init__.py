...
def register_reaching_envs():
    ...
    # VDS Reaching Environment
    # MazeA
    register(
        id='Point2D-Dense-MazeA-v0',
        entry_point='multiworld.envs.pygame.point2d:Point2DWallEnv',
        tags={
            # 'git-commit-hash': '73c8823',
            'author': 'seongun-kim'
        ...
        kwargs={
            ...
            'ball_radius': 0, # # 0.5,
            'target_radius': 0.1, # 0.6,
            'wall_thickness': 0.07692307692307693, # 0.4,
            'boundary_dist': 1, # 6,
            'render_size': 500,

            'randomize_position_on_reset': False,
            'fixed_init_position': (-0.7692308, -0.7692308), # (-5.16666667, 5.16666667),
            'fixed_goal': None, # (0.7692308, 0.7692308), # (5.16666667, -5.16666667),
            # 'fixed_goal': (-4.16666667, 5.16666667),

            'reward_type': 'dense'
        },
    )
    register(
        id='Point2D-Sparse-MazeA-v0',
        entry_point='multiworld.envs.pygame.point2d:Point2DWallEnv',
        tags={
            # 'git-commit-hash': '73c8823',
            'author': 'seongun-kim'
    ...
    register(
        id='Point2D-Image-Dense-MazeA-v0',
        entry_point=point2d_image_dense_mazea_v0,
        tags={
            # 'git-commit-hash': '73c8823',
            'author': 'seongun-kim'
        },
    ...
    register(
        id='Point2D-Image-Sparse-MazeA-v0',
        entry_point=point2d_image_sparse_mazea_v0,
        tags={
            # 'git-commit-hash': '73c8823',
            'author': 'seongun-kim'
        },
    ...
    register(
        ...
        kwargs={
            ...
            'randomize_position_on_reset': False,
            'fixed_init_position': (-0.7692308, 0.7692308), # (-5.16666667, 5.16666667),
            'fixed_goal': None, # (0.30769232, 0.30769232), # (5.16666667, -5.16666667),
            # 'fixed_goal': (-4.16666667, 5.16666667),
...
def register_pnp_envs():
    ...
    for env_id, extra_settings in [
        ...
        new_kwargs = same_size_settings.copy()
        new_kwargs.update(extra_settings)
        register(
            id=env_id,
            entry_point=PickAndPlaceEnv,
...
def point2d_image_dense_mazea_v0(**kwargs):
    import numpy as np
    from multiworld.core.image_env import ImageEnv
    ...
    env = Point2DWallEnv(
        ...
        randomize_position_on_reset=False,
        fixed_init_position=np.array([-5.16666667, 5.16666667]),
        fixed_goal=np.array([5.16666667, -5.16666667]),
...
def point2d_image_sparse_mazea_v0(**kwargs):
    import numpy as np
    from multiworld.core.image_env import ImageEnv
    from multiworld.envs.pygame.point2d import Point2DWallEnv
    env = Point2DWallEnv(
