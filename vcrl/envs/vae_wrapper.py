...
from vcrl.envs.wrappers import ProxyEnv
...
class VAEWrappedEnv(ProxyEnv, MultitaskEnv):
    ...
    def sample_goals(self, batch_size):
        ...
        # self._goal_sampling_mode = 'env'  # Monitor
        ...
        elif self._goal_sampling_mode == 'edl_goal_sampler':
            assert self.vae is not None, 'VAE for sampling goals doesn\'t exist.'
            sample_from_true_prior = self.sample_from_true_prior
            self.sample_from_true_prior = True
            sampled_latents = ptu.from_numpy(self._sample_vae_prior(batch_size))
            sampled_goals = ptu.get_numpy(self.vae.decode(sampled_latents)[0])
            self.sample_from_true_prior = sample_from_true_prior
            return {'desired_goal': sampled_goals}
        else:
