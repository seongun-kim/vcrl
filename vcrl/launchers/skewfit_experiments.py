...
from vcrl.envs.vae_wrapper import temporary_mode

import cv2
import numpy as np
import os.path as osp

from vcrl.samplers.data_collector.vae_env import (
    VAEWrappedEnvPathCollector,
)
from vcrl.torch.her.her import HERTrainer
from vcrl.torch.sac.policies import MakeDeterministic
...
from vcrl.util.video import dump_video
...
def train_vae(variant, return_data=False):
    from vcrl.util.ml_util import PiecewiseLinearSchedule
    from vcrl.torch.vae.conv_vae import (
        ConvVAE,
    )
    import vcrl.torch.vae.conv_vae as conv_vae
...
def get_exploration_strategy(variant, env):
    from vcrl.exploration_strategies.epsilon_greedy import EpsilonGreedy
    from vcrl.exploration_strategies.gaussian_strategy import GaussianStrategy
    from vcrl.exploration_strategies.ou_strategy import OUStrategy
...
def skewfit_experiment(variant):
    import vcrl.torch.pytorch_util as ptu
    from vcrl.data_management.online_vae_replay_buffer import \
        OnlineVaeRelabelingBuffer
    from vcrl.torch.networks import ConcatMlp
    from vcrl.torch.sac.policies import TanhGaussianPolicy
    from vcrl.torch.vae.vae_trainer import ConvVAETrainer
